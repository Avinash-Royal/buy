package com.septems.avinash.ngrid.Messaging.service;
...
//            if (!uid.equals("")) {
                if ("true".equals(StaticConfig.signin)){
                    ...
                    FirebaseDatabase.getInstance().getReference().child("user/" + mAuth.getCurrentUser().getUid() + "/status/isOnline").setValue(false);
                    FirebaseDatabase.getInstance().getReference().child("user/" + mAuth.getCurrentUser().getUid() + "/status/timestamp").setValue(System.currentTimeMillis());
                }
//            }
        }
    }
